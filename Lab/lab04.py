...
def map_then_filter(func_map, func_filter, lst):
    ...
    """
    #YOUR CODE IS HERE#
    mapping = my_map(func_map, lst)
    filted = my_filter(func_filter, mapping)
    return list(filted)
...
def new_matrix(func, matrix):
    ...
    for i in range(len(matrix)):
        out.append(list(my_map(func, matrix[i])))
    return list(out)
...
def pick_me(f, g, num):
    ...
    """
    # YOUR CODE IS HERE
    return lambda x: f(x) if x > num else g(x)
